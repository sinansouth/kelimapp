...
import { getChallenge, getOpenChallenges, getFriends, getAuthInstance, getChallengeHistory, getTournaments, joinTournament, checkTournamentTimeouts } from '../services/supabase';
...
const ChallengeModal: React.FC<ChallengeModalProps> = ({ onClose, onCreateChallenge, onJoinChallenge }) => {
    ...
    const fetchHistory = async () => {
        ...
        if (myUid) {
            try {
                const list = await getChallengeHistory(myUid);
                setHistoryList(list);
            } catch (e) {
                console.error(e);
            }
                                    ...
                                    return (
                                        <div key={t.id} className="p-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <span className={`px-2 py-1 text-[10px] font-bold rounded uppercase ${t.status === 'registration' ? (isNotStartedYet ? 'bg-slate-200 text-slate-600' : 'bg-green-100 text-green-600') :
