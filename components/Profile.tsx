...
import { AVATARS, BADGES, FRAMES, BACKGROUNDS, THEME_COLORS } from '../data/assets';
import { getAuthInstance, logoutUser, checkUsernameExists, updateCloudUsername, syncLocalToCloud, deleteAccount, resetUserPassword, updateUserEmail, addFriend, getFriends, LeaderboardEntry } from '../services/firebase';
...
const Profile: React.FC<ProfileProps> = ({ onBack, onProfileUpdate, onOpenMarket, onLoginRequest, externalStats, showAlert, onViewProfile }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const auth = getAuthInstance();
    if (auth && auth.currentUser && !profile.isGuest) {
  ...
  const handleResetPassword = async () => {
      if (currentUser?.email) {
          try {
              ...
              showAlert("Başarılı", `Şifre sıfırlama bağlantısı ${currentUser.email} adresine gönderildi. (Lütfen spam/gereksiz kutusunu da kontrol ediniz)`, "success");
          } catch (e) {
  ...
  const handleAddFriend = async (e: React.FormEvent) => {
      ...
      try {
          const friendName = await addFriend(currentUser.uid, friendCode.trim().toUpperCase());
  ...
  const onAuthSuccess = () => {
      ...
      if (!userProfile.isGuest && getAuthInstance().currentUser) {
          fetchFriends(getAuthInstance().currentUser!.uid);
      }
  };
                                                ...
                                                >
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        ...
                                                        <div className="min-w-0">
                                                            <div className="text-xs font-bold truncate" style={{color: friendTheme.textMain, fontFamily: friendTheme.fontFamily}}>{friend.name}</div>
