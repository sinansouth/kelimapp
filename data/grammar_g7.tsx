import React from 'react';
import { GrammarTopic } from '../types';

export const GRAMMAR_G7: Record<string, GrammarTopic[]> = {
  'g7u1': [
    {
      title: "Describing People (Physical Appearance)",
      content: (
        <div>
          <p>Dış görünüşü anlatırken <strong>have/has got</strong> veya <strong>is/are</strong> kullanılır.</p>
          <ul className="list-disc pl-4 space-y-1">
            <li>She <strong>is</strong> tall and slim. (O uzun ve zayıftır.)</li>
            <li>He <strong>has got</strong> curly hair. (Onun kıvırcık saçı var.)</li>
            <li>They <strong>have got</strong> hazel eyes. (Onların ela gözleri var.)</li>
          </ul>
        </div>
      )
    },
    {
      title: "Describing Personality (Kişilik)",
      content: (
        <div>
          <p>Kişilik özelliklerini anlatırken:</p>
          <p>She is <strong>generous</strong> because she likes sharing.</p>
          <p>He is <strong>stubborn</strong>; he never changes his mind.</p>
        </div>
      )
    },
    {
      title: "Comparatives (Kişiler Arası)",
      content: (
        <div>
          <p>Ali is <strong>taller than</strong> Ahmet.</p>
          <p>Ayşe is <strong>more outgoing than</strong> Fatma.</p>
        </div>
      )
    }
  ],
  'g7u2': [
    {
      title: "Simple Present Tense",
      content: (
        <div>
          <p>Spor alışkanlıklarını anlatırken:</p>
          <p>I <strong>play</strong> tennis every weekend.</p>
          <p>He <strong>goes</strong> swimming on Fridays.</p>
        </div>
      )
    },
    {
      title: "Frequency Adverbs (Sıklık Zarfları)",
      content: (
        <div>
          <p>Bir eylemi ne sıklıkla yaptığımızı belirtir.</p>
          <ul className="list-disc pl-4">
            <li><strong>Always:</strong> Her zaman</li>
            <li><strong>Usually:</strong> Genellikle</li>
            <li><strong>Often:</strong> Sık sık</li>
            <li><strong>Sometimes:</strong> Bazen</li>
            <li><strong>Never:</strong> Asla</li>
          </ul>
          <p>Örnek: I <strong>never</strong> eat junk food.</p>
        </div>
      )
    }
  ],
  'g7u3': [
    {
      title: "Simple Past Tense (To Be - Was/Were)",
      content: (
        <div>
          <p>Geçmişteki durumlardan bahsederken:</p>
          <p>I <strong>was</strong> born in 2010.</p>
          <p>She <strong>was</strong> a famous scientist.</p>
          <p>They <strong>were</strong> happy.</p>
        </div>
      )
    },
    {
      title: "Simple Past Tense (Regular/Irregular Verbs)",
      content: (
        <div>
          <p>Biyografi anlatırken geçmiş zaman fiilleri kullanılır.</p>
          <p>He <strong>graduated</strong> from university. (Düzenli: -ed)</p>
          <p>She <strong>won</strong> the Nobel Prize. (Düzensiz: Win - Won)</p>
          <p>They <strong>moved</strong> to London.</p>
        </div>
      )
    }
  ],
  'g7u4': [
    {
      title: "Should / Shouldn't (Obligations)",
      content: (
        <div>
          <p>Vahşi yaşamı korumak için yapılması gerekenler:</p>
          <p>We <strong>should</strong> protect wildlife.</p>
          <p>We <strong>should</strong> stop hunting.</p>
          <p>We <strong>shouldn't</strong> destroy habitats.</p>
        </div>
      )
    },
    {
      title: "Simple Present (Facts)",
      content: (
        <div>
          <p>Hayvanlar hakkındaki gerçekleri anlatırken:</p>
          <p>Tigers <strong>live</strong> in jungles.</p>
          <p>Elephants <strong>have</strong> big ears.</p>
        </div>
      )
    }
  ],
  'g7u5': [
    {
      title: "Preferences (Prefer)",
      content: (
        <div>
          <p>Tercihlerimizi belirtirken <strong>prefer</strong> kullanırız.</p>
          <p><strong>Kalıp:</strong> prefer + (noun / Ving) + <strong>to</strong> + (noun / Ving)</p>
          <ul className="list-disc pl-4 mt-2">
            <li>I prefer <strong>comedies</strong> to <strong>horror movies</strong>.</li>
            <li>She prefers <strong>watching</strong> TV to <strong>going</strong> out.</li>
          </ul>
        </div>
      )
    },
    {
      title: "WH- Questions",
      content: (
        <div>
          <p>What kind of movies do you like?</p>
          <p>Who is your favorite actor?</p>
          <p>When is the movie?</p>
        </div>
      )
    }
  ],
  'g7u6': [
    {
      title: "Quantifiers (Miktar Belirleyiciler)",
      content: (
        <div>
          <p>Parti hazırlığı için miktar sorma ve söyleme:</p>
          <ul className="list-disc pl-4">
            <li><strong>Some:</strong> Biraz/Birkaç (Olumlu cümle)</li>
            <li><strong>Any:</strong> Hiç (Olumsuz ve soru)</li>
            <li><strong>A lot of:</strong> Çok</li>
            <li><strong>A few:</strong> Az (Sayılabilen)</li>
            <li><strong>A little:</strong> Az (Sayılamayan)</li>
          </ul>
          <p>We need <strong>a few</strong> balloons and <strong>a little</strong> juice.</p>
        </div>
      )
    },
    {
      title: "Making Arrangements (Plan Yapma)",
      content: (
        <div>
          <p>Why don't we give a party?</p>
          <p>Let's decorate the room.</p>
          <p>Should we order a cake?</p>
        </div>
      )
    }
  ],
  'g7u7': [
    {
      title: "Future Tense (Will)",
      content: (
        <div>
          <p>Gelecekle ilgili tahminlerde bulunurken <strong>will</strong> kullanırız.</p>
          <p>I think I <strong>will</strong> be a doctor.</p>
          <p>You <strong>will</strong> be very rich in the future.</p>
          <p>The world <strong>will</strong> be a better place.</p>
          <p><strong>Won't</strong> (Will not): Olmayacak.</p>
        </div>
      )
    }
  ],
  'g7u8': [
    {
      title: "Public Buildings & Purposes",
      content: (
        <div>
          <p>Nereye, niçin gittiğimizi anlatmak için <strong>to</strong> (için) kullanırız.</p>
          <p>I went to the bakery <strong>to buy</strong> bread. (Ekmek almak için fırına gittim.)</p>
          <p>She went to the police station <strong>to report</strong> a crime.</p>
          <p>We go to the library <strong>to read</strong> books.</p>
        </div>
      )
    }
  ],
  'g7u9': [
    {
      title: "Must / Mustn't (Strong Obligation)",
      content: (
        <div>
          <p>Güçlü zorunluluklar ve yasaklar:</p>
          <p>We <strong>must</strong> protect the environment. (Korumalıyız - Şart)</p>
          <p>We <strong>mustn't</strong> pollute the air. (Kirletmemeliyiz - Yasak)</p>
        </div>
      )
    },
    {
      title: "Have to / Has to",
      content: (
        <div>
          <p>Dışarıdan gelen zorunluluklar:</p>
          <p>We <strong>have to</strong> use public transport to reduce traffic.</p>
        </div>
      )
    }
  ],
  'g7u10': [
    {
      title: "Comparatives (Karşılaştırma)",
      content: (
        <div>
          <p>Jupiter is <strong>larger than</strong> Mars.</p>
          <p>Venus is <strong>hotter than</strong> Earth.</p>
        </div>
      )
    },
    {
      title: "Superlatives (En Üstünlük)",
      content: (
        <div>
          <p>Bir gruptaki "en" özelliği belirtir.</p>
          <p>Jupiter is <strong>the largest</strong> planet.</p>
          <p>Mercury is <strong>the closest</strong> planet to the Sun.</p>
          <p>Neptune is <strong>the farthest</strong> planet.</p>
        </div>
      )
    }
  ]
};